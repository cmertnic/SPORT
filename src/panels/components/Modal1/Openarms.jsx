  ...
  return (
    ...
    flexDirection: "column",
     ...
     <img src={currentImage} alt="" style={{
      ...
      {time <= 30*60 && time >= 25*60 && <h2 style={{textAlign:"center"}}>Упражнение первое: отжимания от скамьи </h2>}
      {time <= 25*60 && time >= 249*6 && <p>Переходим ко 2 упражнению</p>}
      {time <= 25*60 && time >= 20*60 && <h2 style={{textAlign:"center"}}>Упражнение второе: планка на коленях с подъемом рук к плечам</h2>}
      {time <= 20*60 && time >= 199*6 && <p>Переходим к 3 упражнению</p>}
      {time <= 20*60 && time >= 15*60 && <h2 style={{textAlign:"center"}}>Упражнение третье: сгибания и разгибания запястий</h2>}      
      {time <= 15*60 && time >= 149*6 && <p>Переходим к 4 упражнению</p>}
      {time <= 15*60 && time >= 10*60 && <h2 style={{textAlign:"center"}}>Упражнение четвёртое: разгибания гантелей из-за головы</h2>}      
      {time <= 10*60 && time >= 99*6 && <p>Переходим к 5 упражнению</p>}
      {time <= 10*60 && time >= 5*60 && <h2 style={{textAlign:"center"}}>Упражнение пятое: Обратные отжимания от скамьи</h2>}      
      {time <= 5*60 && time >= 49*6 && <p>Переходим к 6 упражнению</p>}
      {time <= 5*60 && time >= 2*60 && <h2 style={{textAlign:"center"}}>Упражнение шестое: Подъём штанги или гантелей на бицепс</h2>}      
      {time <= 2*60 && time === 19*6 && <p>Переходим к 7 упражнению</p>}
      {time <= 2*60 && time >= 0*60 && <h2 style={{textAlign:"center"}}>упражнение седьмое: Отжимания на пальцах</h2>}      
