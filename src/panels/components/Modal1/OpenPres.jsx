...
import Skruchivania from "../../../img/PresGif/Skruchivania.gif"
import Podtygivanie from "../../../img/PresGif/Podtygivanie_nog_polusidya.gif"
...
const Timer = () => {
  ...
  const [currentImage, setCurrentImage] = useState(Skruchivania);

  const [count, setCount] = useState(0);

  useEffect(() => {
    if (time <= 30 * 60 && time >= 25 * 60) {
      setCurrentImage(Skruchivania);
      setCount(setCount + 1);
    } else if (time <= 25 * 60 && time >= 20 * 60) {
      setCurrentImage(Podtygivanie);
      setCount(setCount + 1);
  ...
  return (
    ...
    flexDirection: "column",
     ...
     <img src={currentImage} alt="" style={{
      ...
      {time <= 30*60 && time >= 25*60 && <h2 style={{textAlign:"center"}}>Упражнение первое: Скручивание с подъемом колен</h2>}
      {time <= 25*60 && time >= 249*6 && <p>Переходим ко 2 упражнению</p>}
      {time <= 25*60 && time >= 20*60 && <h2 style={{textAlign:"center"}}>Упражнение второе: Складка с подтягиванием ног</h2>}
      {time <= 20*60 && time >= 199*6 && <p>Переходим к 3 упражнению</p>}
      {time <= 20*60 && time >= 15*60 && <h2 style={{textAlign:"center"}}>Упражнение третье: Шаги из планки в четвереньки</h2>}      
      {time <= 15*60 && time >= 149*6 && <p>Переходим к 4 упражнению</p>}
      {time <= 15*60 && time >= 10*60 && <h2 style={{textAlign:"center"}}>Упражнение четвёртое: Косые скручивания ладонь-стопа</h2>}      
      {time <= 10*60 && time >= 99*6 && <p>Переходим к 5 упражнению</p>}
      {time <= 10*60 && time >= 5*60 && <h2 style={{textAlign:"center"}}>Упражнение пятое: Вертикальные ножницы с руками</h2>}      
      {time <= 5*60 && time >= 49*6 && <p>Переходим к 6 упражнению</p>}
      {time <= 5*60 && time >= 2*60 && <h2 style={{textAlign:"center"}}>Упражнение шестое: Велосипед с поднятым корпусом</h2>}      
      {time <= 2*60 && time === 19*6 && <p>Переходим к 7 упражнению</p>}
      {time <= 2*60 && time >= 0*60 && <h2 style={{textAlign:"center"}}>Упражнение седьмое: Повороты корпуса в планке на локтях</h2>}      
