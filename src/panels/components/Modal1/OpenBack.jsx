...
import Podtyagivania from "../../../img/BackGif/Podtyagivania.gif"
import TGOR from "../../../img/BackGif/Tyaga-ganteli-odnoy-rukoy.gif"
...
const Timerarms = () => {
  ...
  const [currentImage, setCurrentImage] = useState(Podtyagivania);

  const [count, setCount] = useState(0);

  useEffect(() => {
    if (time <= 30 * 60 && time >= 25 * 60) {
      setCurrentImage(Podtyagivania);
      setCount(setCount + 1);
    } else if (time <= 25 * 60 && time >= 20 * 60) {
      setCurrentImage(TGOR);
      setCount(setCount + 1);
  ...
  return (
    ...
    flexDirection: "column",
     ...
     <img src={currentImage} alt="" style={{
      ...
      {time <= 30*60 && time >= 25*60 && <h2 style={{textAlign:"center"}}>Упражнение первое: Подтягивания  </h2>}
      {time <= 25*60 && time >= 249*6 && <p>Переходим ко 2 упражнению</p>}
      {time <= 25*60 && time >= 20*60 && <h2 style={{textAlign:"center"}}>Упражнение второе: Тяга гантели одной рукой в наклоне</h2>}
      {time <= 20*60 && time >= 199*6 && <p>Переходим к 3 упражнению</p>}
      {time <= 20*60 && time >= 15*60 && <h2 style={{textAlign:"center"}}>Упражнение третье: Пожимания плечами (гантели)</h2>}      
      {time <= 15*60 && time >= 149*6 && <p>Переходим к 4 упражнению</p>}
      {time <= 15*60 && time >= 10*60 && <h2 style={{textAlign:"center"}}>Упражнение четвёртое: Наклонные подтягивания</h2>}      
      {time <= 10*60 && time >= 99*6 && <p>Переходим к 5 упражнению</p>}
      {time <= 10*60 && time >= 5*60 && <h2 style={{textAlign:"center"}}>Упражнение пятое: Обратные махи гантелями лёжа на животе</h2>}      
      {time <= 5*60 && time >= 49*6 && <p>Переходим к 6 упражнению</p>}
      {time <= 5*60 && time >= 0*60 && <h2 style={{textAlign:"center"}}>Упражнение шестое: IYT-подъёмы</h2>}         
