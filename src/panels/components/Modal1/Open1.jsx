...
import Prisedsvipadom from "../../../img/gif/prisedsvipadom.gif"
import Pezinka_most from "../../../img/gif/rezinka_most.gif"
import Maxinogami from "../../../img/gif/maxinogami.gif"
import Planka from "../../../img/gif/planka.gif"
import Tiagabedro from "../../../img/gif/tiagabedro.gif"
import Prisedsvitrukami from "../../../img/gif/prisedsvitrukami.gif"
  ...
  return (
    ...
    flexDirection: "column",
     ...
     <img src={currentImage} alt="" style={{
      ...
      {time <= 30*60 && time >= 25*60 && <h2 style={{textAlign:"center"}}>Упражнение первое: берпи</h2>}
      {time <= 25*60 && time >= 249*6 && <p>Переходим ко 2 упражнению</p>}
      {time <= 25*60 && time >= 20*60 && <h2 style={{textAlign:"center"}}>Упражнение второе: Ягодичный мостик с резинкой</h2>}
      {time <= 20*60 && time >= 199*6 && <p>Переходим к 3 упражнению</p>}
      {time <= 20*60 && time >= 15*60 && <h2 style={{textAlign:"center"}}>Упражнение третье: Приседание в выпаде с опорой ноги сзади</h2>}      
      {time <= 15*60 && time >= 149*6 && <p>Переходим к 4 упражнению</p>}
      {time <= 15*60 && time >= 10*60 && <h2 style={{textAlign:"center"}}>Упражнение четвёртое:  Махи ногами на четвереньках</h2>}      
      {time <= 10*60 && time >= 99*6 && <p>Переходим к 5 упражнению</p>}
      {time <= 10*60 && time >= 5*60 && <h2 style={{textAlign:"center"}}>Упражнение пятое: Тяга бедрами</h2>}      
      {time <= 5*60 && time >= 49*6 && <p>Переходим к 6 упражнению</p>}
      {time <= 5*60 && time >= 2*60 && <h2 style={{textAlign:"center"}}>Упражнение шестое: Планка</h2>}      
      {time <= 2*60 && time === 19*6 && <p>Переходим к 7 упражнению</p>}
      {time <= 2*60 && time >= 0*60 && <h2 style={{textAlign:"center"}}>Упражнение седьмое: Приседания с вытянутыми руками</h2>}      
